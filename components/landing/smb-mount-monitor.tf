...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "smb_mount_failures" {
  ...
  name                = "ingest${each.value.landing_zone_key}-doc-migration-smb-mount-failures-${var.env}"
  ...
  description         = "Repeated SMB/CIFS mount health-check failures on ingest00-doc-migration-prod."
  severity            = 2
  enabled             = true
  ...
  criteria {
    query                   = <<-KQL
      Syslog
      | where Computer =~ "ingest00-doc-migration-prod"
